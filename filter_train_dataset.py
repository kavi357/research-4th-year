...
OUT_FILE = OUTPUT_PATH / "training_features_no_covers80.npz"

EXCLUDE_DATASETS = {"covers80"}

print("Using DB:", DB_PATH)
print("Excluding datasets:", EXCLUDE_DATASETS)


# --------------------------------------------------
# STEP 1: LOAD DATA FROM SQLITE (FILTERED)
# --------------------------------------------------
def load_rows():
    ...
    cur.execute("""
        ...
        WHERE t.dataset NOT IN ({})
    """.format(",".join("?" * len(EXCLUDE_DATASETS))),
    tuple(EXCLUDE_DATASETS))

    rows = cur.fetchall()
    conn.close()

    print(f"Loaded {len(rows)} tracks (Covers80 excluded)")
    return rows
...
def build_feature_vector(row):
    _, fused_blob, tempo, mfcc_blob, chroma_blob, pitch_blob, _ = row
